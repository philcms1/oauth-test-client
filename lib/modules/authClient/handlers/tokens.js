...
module.exports.getTokens = function(request, reply) {
    Models
        .findTokensByClientAndProviderIds(request.server.app.active_client.client_id, request.server.app.active_provider._id)
        .then((tokens) => {
...
module.exports.postToken = function(request, reply) {
    ...
    request.log(['client-token'], `Obtaining data for grant type ${grant_type}.`);
...
module.exports.tokenCallback = function(request, reply) {
    ...
    switch(request.server.app.selected_grant_type) {
        case 'authorization_code':
            ...
            ProviderServices.getToken(request.server.app.active_provider.token_endpoint, qs.stringify(data), config)
                .then(response => {
                    request.log(['test-callback'], `Successfully got token from provider: ${JSON.stringify(response.data)}.`);
                    request.server.app.active_token = response.data;
                    request.server.app.active_token.client_id = request.server.app.active_client.client_id;
                    request.server.app.active_token.provider_id = request.server.app.active_provider.provider_id;
                    request.log(['test-callback'], `Persisting token: ${JSON.stringify(request.server.app.active_token)}.`);
                ...
                .catch(error => {
                    request.log(['test-callback-error'], `Error getting token from provider: ${error}.`);
                    return reply.view('error', {
                        error_message: `Unable to obtain token: ${error.message}`,
                        user: request.auth.credentials
                    });
                });
            /*wreck.post(
