...
exports.register = (server, options, next) => {
    ...
    server.route({
        ...
        handler: (request, reply) => {
            ...
            if (server.app.active_client.grant_types.indexOf(grant_type) === -1) {
            ...
            server.app.selected_grant_type = grant_type;
            request.log(['client-token'], `Obtaining token for grant type ${grant_type}.`);

            server.app.runtime_state = Randomstring.generate(25);

            const client_data = {
                client_id: server.app.active_client.client_id,
                redirect_uri: server.app.active_client.redirect_uris[0],
                state: server.app.runtime_state
            };
            if (server.app.active_client.scope) {
                client_data.scope = server.app.active_client.scope;
            }
            ...
            switch(grant_type) {
                case 'authorization_code':
                    ...
                    const redirect_authorization_url = `${server.app.active_provider.authorization_endpoint}?${query_parameters}`;
    ...
    server.route({
        ...
        handler: (request, reply) => {
            ...
            if (state && server.app.runtime_state && server.app.runtime_state !== state) {
                request.log(['client-callback-error'], `Invalid state. Expected ${server.app.runtime_state}, got ${state}.`);
                return reply.view('error', {
            ...
            switch(server.app.selected_grant_type) {
                case 'authorization_code':
                    ...
                    const headers = {
                        ...
                        'Authorization': 'Basic ' + encodeClientCredentials(server.app.active_client.client_id, server.app.active_client.client_secret)
                    };

                    wreck.post(
                        server.app.active_provider.token_endpoint,
                        {
                            payload: form_data,
                            headers: headers
                        },
                        (err, res, payload) => {
                            ...
                            server.app.active_token = payload;
